...
- **Jeton long terme** : un `CLAUDE_CODE_OAUTH_TOKEN` ne fonctionne pas, il n'a pas la portée `user:profile` requise par l'API d'utilisation. Seuls conviennent les identifiants de session que Claude Code dépose dans le trousseau, ou à défaut dans `~/.claude/.credentials.json`.
- **Plusieurs entrées de trousseau** : un même Mac en accumule sous le service `Claude Code-credentials`, une par compte connecté. `ClaudeCredentials` les énumère par attributs, puis lit la plus récemment modifiée : c'est celle qu'un `claude login` vient d'écrire. Ne pas revenir à une requête `kSecMatchLimitOne` sans compte, elle en renvoie une au hasard et la reconnexion ne répare alors rien.
